...
export const UiToggleSwitch:React.FC<props>=({title,formData,setFormData,id,name})=>{
  
    return(
        <label htmlFor="yellow-toggle" className="inline-flex relative items-center mr-5 cursor-pointer">
            <input type="checkbox" value="" id="yellow-toggle" className="sr-only peer" checked={formData[name]} onChange={(e)=>setFormData({...formData,[name]:e.target.checked})}/>
